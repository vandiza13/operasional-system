...
export default function ResetPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Status Card */}
      <div className="bg-slate-800/50 rounded-3xl shadow-lg border border-slate-700/50 p-6 md:p-8 relative overflow-hidden backdrop-blur-sm">
        ...
        {loading && status === null ? (
          ...
          <div className="space-y-6">
            ...
            {status.users && status.users.length > 0 && (
              <div>
                <h4 className="text-sm font-bold text-slate-400 uppercase tracking-wider mb-3">Pengguna di Database</h4>
                    ...
                    <tbody className="divide-y divide-slate-800/50">
                      {status.users.map((user) => (
                        <tr key={user.id} className="hover:bg-slate-800/50 transition-colors">
                          ...
                          <td className="p-4">
                            <span className={`text-xs font-bold uppercase px-2.5 py-1 rounded-lg ${user.role === 'SUPER_ADMIN' ? 'bg-rose-500/10 text-rose-400 border border-rose-500/20' :
      ...
      <div className="bg-rose-950/30 rounded-3xl shadow-lg border border-rose-500/30 p-6 md:p-8 relative overflow-hidden">
        ...
        <p className="text-rose-400/80 font-medium mb-6">
          Ini akan MENGHAPUS semua user, expenses, attachments, kategori, ledger, dan payout. Tindakan ini TIDAK BISA dibatalkan!
        </p>
        ...
        <div className="flex flex-col sm:flex-row gap-4">
          <button
            ...
            className={`px-6 py-3 rounded-xl font-bold text-sm transition-all ${confirmed && !loading
